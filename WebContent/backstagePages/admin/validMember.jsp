      			...
      			<ul class="nav">
        			...
        			<li class="active">
          				...
          				<ul class="collapse control-menu in" id="memberControl">
            				...
            				<li class="active"><a href="adminShowMemberAble?deptId=0">现有成员</a></li>
						...
						<div class="btn-group btn-group-justified" role="group" >
  							<s:iterator value="#request.departments">
								<div class="btn-group" role="group">
    								<a href="adminShowMemberAble?deptId=${deptId }"  class="btn btn-default <s:if test="deptId==#parameters.deptId[0]">active</s:if>">${deptName }</a>
  								</div>
							</s:iterator>
						</div>
					</div>
					<div class="manage-cont" >
  							...
  							<tbody>
  								<s:iterator value="#request.pageBeanMember.member" status="m">
  									<tr>
  										...
  										<td>
  											<a href="adminShowMemberById?id=${id }" target="_blank" class="btn btn-info" title="查看基本信息"><i class="fa fa-search"></i></a>
  												...
  												<div class="modal-dialog" role="document">
								    				<div class="modal-content">
      													...
      													<div class="modal-footer">
      														<form action="adminShowMemberAbleOut">
						...
						<s:iterator value="#request.pageBeanMember">
							<div class="paging">
								...
								<s:else>
									<a href="adminShowMemberAble?deptId=<s:property value="#parameters.deptId[0]"/>&page=1">首页</a>
									<a href="adminShowMemberAble?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="%{cp-1}"/>">上一页</a>
								</s:else>
								...
								<s:else>
									<a href="adminShowMemberAble?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="#i"/>">${i }</a>
								</s:else>
								</s:iterator>
								
								<s:if test="%{cp < tp}">
									<a href="adminShowMemberAble?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="%{cp+1}"/>">下一页</a>
									<a href="adminShowMemberAble?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="tp"/>">尾页</a>
								</s:if>
								<s:else>
 					        		<span>下一页</span>
									<span>尾页</span>
