...
<html>
	<head>
		...
		<title>管理后台-系统管理员-第二次面试-已调剂</title>
				...
				<div class="form-table col-sm-12">
					<div class="manage-menu clearfix">
						<ul class="clearfix">
							...
							<li class="active"><a href="adminHandleFourTwoAdjusted?deptId=0">已调剂</a></li>
						</ul>
					</div>
						...
						<div class="btn-group btn-group-justified" role="group" >
  							<s:iterator value="#request.departments">
								<div class="btn-group" role="group">
    								<a href="adminHandleFourTwoAdjusted?deptId=${deptId }"  class="btn btn-default <s:if test="deptId==#parameters.deptId[0]">active</s:if>">${deptName }</a>
  								</div>
							</s:iterator>
						</div>
					</div>
					<div class="searchCont clearfix">
						<form action="adminHandleFourTwoAdjusted" method="get">
					...
					<div class="manage-cont" >
  											...
  											<div class="modal fade" id="<s:property value="%{#m.getIndex()+1}"/>singleModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
									  			<div class="modal-dialog" role="document">
									    			<div class="modal-content">
									      				...
									      				<form action="emailSingleFourAdjustedAdmin">
						...
						<s:iterator value="#request.pageBean">
							<div class="paging">
								...
								<s:else>
									<a href="adminHandleFourTwoAdjusted?deptId=<s:property value="#parameters.deptId[0]"/>&page=1&name=<s:property value="#parameters.name[0]"/>">首页</a>
									<a href="adminHandleFourTwoAdjusted?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="%{cp-1}"/>&name=<s:property value="#parameters.name[0]"/>">上一页</a>
								</s:else>
								...
								<s:else>
									<a href="adminHandleFourTwoAdjusted?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="#i"/>&name=<s:property value="#parameters.name[0]"/>">${i }</a>
								</s:else>
								</s:iterator>
								
								<s:if test="%{cp < tp}">
									<a href="adminHandleFourTwoAdjusted?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="%{cp+1}"/>&name=<s:property value="#parameters.name[0]"/>">下一页</a>
									<a href="adminHandleFourTwoAdjusted?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="tp"/>&name=<s:property value="#parameters.name[0]"/>">尾页</a>
								</s:if>
								<s:else>
 					        		<span>下一页</span>
									<span>尾页</span>
		...
		<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  			<div class="modal-dialog" role="document">
    			<div class="modal-content">
      				...
      				<form action="emailFourAdjustedAdmin">
      					<input type="hidden" name="deptId" value="<s:property value="#parameters.deptId[0]"/>">
      					<div class="modal-body">
        					<div class="emailHead">
