      			...
      			<ul class="nav">
        			...
        			<li class="active">
          				...
          				<ul class="collapse control-menu in" id="memberControl">
            				...
            				<li class="active"><a href="adminShowMemberDisable?deptId=0">已注销成员</a></li>
          				</ul>
        			</li>
        			<li>
						...
						<div class="btn-group btn-group-justified" role="group" >
							<s:iterator value="#request.departments">
								<div class="btn-group" role="group">
    								<a href="adminShowMemberDisable?deptId=${deptId }"  class="btn btn-default <s:if test="deptId==#parameters.deptId[0]">active</s:if>">${deptName }</a>
  								</div>
							</s:iterator>
						</div>
					</div>
					<div class="manage-cont" >
  							...
  							<tbody>
  								<s:iterator value="#request.pageBeanMember.member" status="m">
  									<tr>
  										...
  										<td>
  											<a href="adminShowMemberById?id=${id }" target="_blank" class="btn btn-info" title="查看信息"><i class="fa fa-search"></i></a>
  											<a href="adminShowMemberDisablePass?id=${id }&deptId=<s:property value="#parameters.deptId[0]"/>" class="btn btn-success" title="恢复成员" ><i class="fa fa-refresh"></i></a>
  										</td>
  									</tr>
  								</s:iterator>
  							</tbody>
						...
						<s:iterator value="#request.pageBeanMember">
							<div class="paging">
								...
								<s:else>
									<a href="adminShowMemberDisable?deptId=<s:property value="#parameters.deptId[0]"/>&page=1">首页</a>
									<a href="adminShowMemberDisable?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="%{cp-1}"/>">上一页</a>
								</s:else>
								...
								<s:else>
									<a href="adminShowMemberDisable?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="#i"/>">${i }</a>
								</s:else>
								</s:iterator>
								
								<s:if test="%{cp < tp}">
									<a href="adminShowMemberDisable?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="%{cp+1}"/>">下一页</a>
									<a href="adminShowMemberDisable?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="tp"/>">尾页</a>
								</s:if>
								<s:else>
 					        		<span>下一页</span>
									<span>尾页</span>
