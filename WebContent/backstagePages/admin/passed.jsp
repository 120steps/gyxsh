...
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/css/backstageCss/applicationFormManage.css" />
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/css/backstageCss/departmentMenu.css" />
</head>
<body>
				...
				<div class="user-menu pull-right">
					<div class="dropdown">
						...
						<ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
							...
							<li><a
								href="${pageContext.request.contextPath }/backstagePages/admin/modifyPassword.jsp"><i
	...
	<div class="collapse navbar-collapse leftMenu col-sm-3 col-lg-2"
		...
		<div class="leftMenuCont">
			<ul class="nav">
				<li class="active"><a href="#applicationFormControl"
					data-toggle="collapse" role="button" aria-haspopup="true"
					aria-expanded="false">报名表管理<i class="fa fa-chevron-up"></i></a>
					<ul class="collapse control-menu in" id="applicationFormControl">
						...
						<li class="active"><a href="adminHandleEightFour?deptId=0">已通过</a></li>
						<li><a href="adminHandleFailFour?deptId=0">未通过</a></li>
					</ul></li>
				<li><a href="#memberControl" data-toggle="collapse"
					role="button" aria-haspopup="true" aria-expanded="false">成员管理<i
				...
				<li><a href="#applySysControl" data-toggle="collapse"
					role="button" aria-haspopup="true" aria-expanded="false">报名系统管理<i
						class="fa fa-chevron-down"></i></a>
					...
					<div class="btn-group btn-group-justified" role="group">
						<s:iterator value="#request.departments">
							<div class="btn-group" role="group">
								<a href="adminHandleEightFour?deptId=${deptId }"
									class="btn btn-default <s:if test="deptId==#parameters.deptId[0]">active</s:if>">${deptName }</a>
							</div>
						</s:iterator>
					</div>
				</div>
				<div class="searchCont clearfix">
					<form action="adminHandleEightFour">
						<div class="search-form">
							<input type="hidden" name="deptId"
								value='<s:property value="#parameters.deptId[0]"/>'> <input
								type="text" placeholder="请输入想搜索的申请者姓名..." name="name" />
				...
				<div class="manage-cont">
						...
						<tbody>
							<s:iterator value="#request.pageBean.applicants" status="m">
								<tr>
									...
									<td><s:if test="aptEmailed==1">
											<a href="adminAptShow?id=${id}" target="_blank"
												class="btn btn-info" title="查看报名表"><i
												class="fa fa-search"></i></a>
											<a href="#" class="btn btn-primary" title="设为部门成员"
												data-toggle="modal"
												data-target="#<s:property value="%{#m.getIndex()+1}" />singleModal"><i
												class="fa fa-user-plus"></i></a>
											<div class="modal fade"
												id="<s:property value="%{#m.getIndex()+1}" />singleModal"
												...
												<div class="modal-dialog" role="document">
													<div class="modal-content">
														...
														<div class="modal-body">
															设为部门成员之后你将会在成员管理中看到其相关信息，在报名表管理中的信息将删除！</div>
														<div class="modal-footer">
															<form action="adminHandleEightFourTurn">
																<input type="hidden" name="deptId"
																	value="<s:property value="#parameters.deptId[0]"/>">
					...
					<div class="btnCont clearfix">
						<p class="pull-left">
							<button type="button" class="btn btn-primary" data-toggle="modal"
								data-target="#myModal">全部设为部门成员</button>
						</p>
					</div>
					<s:iterator value="#request.pageBean">
						<div class="paging">
							...
							<s:else>
								<a
									href="adminHandleEightFour?deptId=<s:property value="#parameters.deptId[0]"/>&page=1&name=<s:property value="#parameters.name[0]"/>">首页</a>
								<a
									href="adminHandleEightFour?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="%{cp-1}"/>&name=<s:property value="#parameters.name[0]"/>">上一页</a>
							</s:else>
							...
							<s:iterator var="i" begin="begin" end="end">
								...
								<s:else>
									<a
										href="adminHandleEightFour?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="#i"/>&name=<s:property value="#parameters.name[0]"/>">${i }</a>
								</s:else>
							</s:iterator>

							<s:if test="%{cp < tp}">
								<a
									href="adminHandleEightFour?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="%{cp+1}"/>&name=<s:property value="#parameters.name[0]"/>">下一页</a>
								<a
									href="adminHandleEightFour?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="tp"/>&name=<s:property value="#parameters.name[0]"/>">尾页</a>
							</s:if>
							<s:else>
								<span>下一页</span>
								<span>尾页</span>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					设为部门成员之后你将会在成员管理中看到其相关信息，在报名表管理中的信息将删除！</div>
				<div class="modal-footer">
					<form action="adminHandleEightFourTurnAll">
						<input type="hidden" name="deptId"
							value="<s:property value="#parameters.deptId[0]"/>">
						<button type="submit" class="btn btn-primary">确认</button>
					</form>
