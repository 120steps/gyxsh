...
<html>
	<head>
		...
		<title>管理后台-${session_user.student.merDepartment.deptName }-第一次面试-已通过</title>
			...
			<div class="row right-bar-cont">
				<div class="form-table col-sm-12">
					<div class="manage-menu clearfix">
						<ul class="clearfix">
							<li><a href="secHandleTwoOne">未处理</a></li>
							<li class="active"><a href="secHandleFourOne">已通过</a></li>
							<li><a href="secHandleFiveOne">未通过</a></li>
							<li><a href="secHandleTwoOneAdjusted">已调剂</a></li>
						</ul>
					</div>
					<div class="searchCont clearfix">
						<form action="secHandleFourOne" method="get">
					...
					<div class="manage-cont" >
  									...
  									<tr>
  										...
  										<td>
  											...
  											<s:if test="aptEmailed==0">
  												<a href="secHandleFourOneOut?id=${id}&name=<s:property value="#parameters.name[0]"/>" class="btn btn-warning" title="不通过"><i class="fa fa-frown-o"></i></a>
  											</s:if>
  											...
  											<div class="modal fade" id="<s:property value="%{#m.getIndex()+1}"/>singleModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
									  			<div class="modal-dialog" role="document">
									    			<div class="modal-content">
									      				...
									      				<form action="emailSingleFourSec">
						...
						<s:iterator value="#request.pageBean">
							<div class="paging">
								...
								<s:else>
									<a href="secHandleFourOne?page=1&name=<s:property value="#parameters.name[0]"/>">首页</a>
									<a href="secHandleFourOne?page=<s:property value="%{cp-1}"/>&name=<s:property value="#parameters.name[0]"/>">上一页</a>
								</s:else>
								...
								<s:else>
									<a href="secHandleFourOne?page=<s:property value="#i"/>&name=<s:property value="#parameters.name[0]"/>">${i }</a>
								</s:else>
								</s:iterator>
								
								<s:if test="%{cp < tp}">
									<a href="secHandleFourOne?page=<s:property value="%{cp+1}"/>&name=<s:property value="#parameters.name[0]"/>">下一页</a>
									<a href="secHandleFourOne?page=<s:property value="tp"/>&name=<s:property value="#parameters.name[0]"/>">尾页</a>
								</s:if>
								<s:else>
 					        		<span>下一页</span>
									<span>尾页</span>
