...
<html>
	<head>
		...
		<title>管理后台-${session_user.student.merDepartment.deptName }-已通过</title>
      			...
      			<ul class="nav">
      				<li class="active">
          				...
          				<ul class="collapse control-menu in" id="applicationFormControl">
            				...
            				<li class="active"><a href="secHandleEightFour">已通过</a></li>
			...
			<div class="row right-bar-cont">
				<div class="form-table col-sm-12">
					<div class="searchCont clearfix">
						<form action="secHandleEightFour">
					...
					<div class="manage-cont" >
												...
												<div class="modal-dialog" role="document">
													<div class="modal-content">
														...
														<div class="modal-footer">
															<form action="secHandleEightFourTurn">
						...
						<div class="btnCont clearfix">
							<p class="pull-left">
								<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#myModal">全部设为部门成员</button>
							</p>
						</div>
						...
						<div class="paging">
							...
							<s:else>
								<a
									href="secHandleEightFour?page=1&name=<s:property value="#parameters.name[0]"/>">首页</a>
								<a
									href="secHandleEightFour?page=<s:property value="%{cp-1}"/>&name=<s:property value="#parameters.name[0]"/>">上一页</a>
							</s:else>
							...
							<s:iterator var="i" begin="begin" end="end">
								...
								<s:else>
									<a
										href="secHandleEightFour?page=<s:property value="#i"/>&name=<s:property value="#parameters.name[0]"/>">${i }</a>
								</s:else>
							</s:iterator>

							<s:if test="%{cp < tp}">
								<a
									href="secHandleEightFour?page=<s:property value="%{cp+1}"/>&name=<s:property value="#parameters.name[0]"/>">下一页</a>
								<a
									href="secHandleEightFour?page=<s:property value="tp"/>&name=<s:property value="#parameters.name[0]"/>">尾页</a>
							</s:if>
							<s:else>
								<span>下一页</span>
								<span>尾页</span>
		...
		<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  			<div class="modal-dialog" role="document">
    			<div class="modal-content">
      				...
      				<div class="modal-body">
        				设为部门成员之后你将会在成员管理中看到其相关信息，在报名表管理中的信息将删除！
      				</div>
      				<div class="modal-footer">
        				<form action="secHandleEightFourTurnAll">
							<button type="submit" class="btn btn-primary">确认</button>
						</form>
      				</div>
    			</div>
