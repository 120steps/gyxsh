...
<html>
	<head>
		...
		<title>管理后台-主席团-第二次面试-未处理</title>
      			...
      			<ul class="nav">
      				<li class="active">
          				...
          				<ul class="collapse control-menu in" id="applicationFormControl">
            				...
            				<li class="active"><a href="merHandleFourTwo?deptId=0">第二次面试</a></li>
			...
			<div class="row right-bar-cont">
				<div class="form-table col-sm-12">
					<div class="manage-menu clearfix">
						<ul class="clearfix">
							<li class="active"><a href="merHandleFourTwo?deptId=0">未处理</a></li>
							<li><a href="merHandleSixTwo?deptId=0">已通过</a></li>
							<li><a href="merHandleSevenTwo?deptId=0">未通过</a></li>
							<li><a href="merHandleFourTwoAdjusted?deptId=0">已调剂</a></li>
						</ul>
					</div>
						...
						<div class="btn-group btn-group-justified" role="group" >
  							<s:iterator value="#request.departments">
								<div class="btn-group" role="group">
    								<a href="merHandleFourTwo?deptId=${deptId }"  class="btn btn-default <s:if test="deptId==#parameters.deptId[0]">active</s:if>">${deptName }</a>
  								</div>
							</s:iterator>
						</div>
					</div>
					<div class="searchCont clearfix">
						<form action="merHandleFourTwo">
					...
					<div class="manage-cont ">
  									...
  									<tr>
  										...
  										<td class="operationTd">
  											<s:if test="aptEmailed==1">
  												...
  												<a href="merHandleFourTwoPass?id=${id}&deptId=<s:property value="#parameters.deptId[0]"/>&name=<s:property value="#parameters.name[0]"/>" class="btn btn-success" title="通过"><i class="fa fa-smile-o"></i></a>
  												<a href="merHandleFourTwoOut?id=${id}&deptId=<s:property value="#parameters.deptId[0]"/>&name=<s:property value="#parameters.name[0]"/>" class="btn btn-warning" title="不通过"><i class="fa fa-frown-o"></i></a>
  												...
  												<div class="adjustCont collapse " id="adjustItem<s:property value="%{#m.getIndex()+1}"/>">
  													<form action="merHandleFourTwoAdjust">
						...
						<s:iterator value="#request.pageBean">
							<div class="paging">
								...
								<s:else>
									<a href="merHandleFourTwo?deptId=<s:property value="#parameters.deptId[0]"/>&page=1&name=<s:property value="#parameters.name[0]"/>">首页</a>
									<a href="merHandleFourTwo?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="%{cp-1}"/>&name=<s:property value="#parameters.name[0]"/>">上一页</a>
								</s:else>
								...
								<s:else>
									<a href="merHandleFourTwo?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="#i"/>&name=<s:property value="#parameters.name[0]"/>">${i }</a>
								</s:else>
								</s:iterator>
								
								<s:if test="%{cp < tp}">
									<a href="merHandleFourTwo?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="%{cp+1}"/>&name=<s:property value="#parameters.name[0]"/>">下一页</a>
									<a href="merHandleFourTwo?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="tp"/>&name=<s:property value="#parameters.name[0]"/>">尾页</a>
								</s:if>
								<s:else>
 					        		<span>下一页</span>
									<span>尾页</span>
