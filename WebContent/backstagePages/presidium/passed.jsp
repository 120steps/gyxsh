			...
			<div class="user-menu pull-right">
				<div class="dropdown">
					...
					<ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
						...
						<li><a href="merModifyBasicInfo"><i class="fa fa-cog"></i>
								设置</a></li>
			...
			<ul class="nav">
				<li class="active"><a href="#applicationFormControl"
					...
					<ul class="collapse control-menu in" id="applicationFormControl">
						...
						<li class="active"><a href="merHandleEightFour?deptId=0">已通过</a></li>
		...
		<div class="row right-bar-cont">
			<div class="form-table col-sm-12">
					...
					<div class="btn-group btn-group-justified" role="group">
						<s:iterator value="#request.departments">
							<div class="btn-group" role="group">
								<a href="merHandleEightFour?deptId=${deptId }"
				...
				<div class="searchCont clearfix">
					<form action="merHandleEightFour">
				...
				<div class="manage-cont">
							...
							<s:iterator value="#request.pageBean.applicants" status="m">
								<tr>
									...
									<td><s:if test="aptEmailed==1">
											<a href="merAptShow?id=${id}" target="_blank"
												...
												<div class="modal-dialog" role="document">
													<div class="modal-content">
														...
														<div class="modal-footer">
															<form action="merHandleEightFourTurn">
																<input type="hidden" name="deptId"
					...
					<div class="btnCont clearfix">
						<p class="pull-left">
							...
							<button type="button" class="btn btn-primary" data-toggle="modal"
								data-target="#allModal">全部设为部门成员</button>
						</p>
					</div>
					<s:iterator value="#request.pageBean">
						<div class="paging">
							...
							<s:else>
								<a
									href="merHandleEightFour?deptId=<s:property value="#parameters.deptId[0]"/>&page=1&name=<s:property value="#parameters.name[0]"/>">首页</a>
								<a
									href="merHandleEightFour?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="%{cp-1}"/>&name=<s:property value="#parameters.name[0]"/>">上一页</a>
							</s:else>
							...
							<s:iterator var="i" begin="begin" end="end">
								...
								<s:else>
									<a
										href="merHandleEightFour?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="#i"/>&name=<s:property value="#parameters.name[0]"/>">${i }</a>
								</s:else>
							</s:iterator>

							<s:if test="%{cp < tp}">
								<a
									href="merHandleEightFour?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="%{cp+1}"/>&name=<s:property value="#parameters.name[0]"/>">下一页</a>
								<a
									href="merHandleEightFour?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="tp"/>&name=<s:property value="#parameters.name[0]"/>">尾页</a>
							</s:if>
							<s:else>
								<span>下一页</span>
								<span>尾页</span>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					设为部门成员之后你将会在成员管理中看到其相关信息，在报名表管理中的信息将删除！</div>
				<div class="modal-footer">
					<form action="merHandleEightFourTurnAll">
						<input type="hidden" name="deptId"
							value="<s:property value="#parameters.deptId[0]"/>">
						<button type="submit" class="btn btn-primary">确认</button>
					</form>
