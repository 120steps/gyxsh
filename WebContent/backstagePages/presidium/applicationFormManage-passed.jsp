...
<html>
	<head>
		...
		<title>管理后台-主席团-报名表审核-已通过</title>
      			...
      			<ul class="nav">
      				<li class="active">
          				...
          				<ul class="collapse control-menu in" id="applicationFormControl">
           					<li class="active"><a href="merHandleOne?deptId=0">报名表审核</a></li>
			...
			<div class="row right-bar-cont">
				<div class="form-table col-sm-12">
					<div class="manage-menu clearfix">
						<ul class="clearfix">
							<li><a href="merHandleOne?deptId=0">未处理</a></li>
							<li class="active"><a href="merHandleTwo?deptId=0">已通过</a></li>
							<li><a href="merHandleThree?deptId=0">未通过</a></li>
						</ul>
					</div>
						...
						<div class="btn-group btn-group-justified" role="group" >
  							<s:iterator value="#request.departments">
								<div class="btn-group" role="group">
    								<a href="merHandleTwo?deptId=${deptId }"  class="btn btn-default <s:if test="deptId==#parameters.deptId[0]">active</s:if>">${deptName }</a>
  								</div>
							</s:iterator>
						</div>
					</div>
					<div class="searchCont clearfix">
						<form action="merHandleTwo" method="get">
					...
					<div class="manage-cont" >
						<table class="table table-hover">
  							<thead>
  								<tr>
  									<!--<th><input id="passedAllCheck" type="checkbox" /> <label class="allLabel" for="passedAllCheck">全选</label></th>-->
  									<th>#</th>
  									<th>姓名</th>
  									<th>专业班级</th>
  									<th>第一志愿</th>
  								...
  								<s:iterator value="#request.pageBean.applicants" status="m">
  									<tr>
  										...
  										<td>
  											...
  											<s:if test="aptEmailed==0">
  												<a href="merHandleTwoOut?id=${id}&deptId=<s:property value="#parameters.deptId[0]"/>&name=<s:property value="#parameters.name[0]"/>" class="btn btn-warning" title="不通过"><i class="fa fa-frown-o"></i></a>
  											</s:if>
						...
						<div class="btnCont clearfix">
							<p class="pull-left">
								<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#allModal">一键通知</button>
							</p>
						</div>
						<s:iterator value="#request.pageBean">
							<div class="paging">
								...
								<s:else>
									<a href="merHandleTwo?deptId=<s:property value="#parameters.deptId[0]"/>&page=1&name=<s:property value="#parameters.name[0]"/>">首页</a>
									<a href="merHandleTwo?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="%{cp-1}"/>&name=<s:property value="#parameters.name[0]"/>">上一页</a>
								</s:else>
								...
								<s:else>
									<a href="merHandleTwo?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="#i"/>&name=<s:property value="#parameters.name[0]"/>">${i }</a>
								</s:else>
								</s:iterator>
								
								<s:if test="%{cp < tp}">
									<a href="merHandleTwo?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="%{cp+1}"/>&name=<s:property value="#parameters.name[0]"/>">下一页</a>
									<a href="merHandleTwo?deptId=<s:property value="#parameters.deptId[0]"/>&page=<s:property value="tp"/>&name=<s:property value="#parameters.name[0]"/>">尾页</a>
								</s:if>
								<s:else>
 					        		<span>下一页</span>
									<span>尾页</span>
    			...
    			<div class="modal-content">
      				...
      				<form action="emailTwoMer">
      					...
      					<div class="modal-body">
        					...
        					<div class="emailCont">
        						<textarea id="emailContent" rows="8" class="form-control" name="emailText"></textarea>
        					</div>
        					<div class="emailFooter clearfix">
        						<p class="pull-right">管理学院学生会</p>
        					</div>
